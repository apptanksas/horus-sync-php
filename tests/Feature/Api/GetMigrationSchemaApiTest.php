...
class GetMigrationSchemaApiTest extends ApiTestCase
{
    ...
    function testGetMigrationSchemaIsSuccess()
    {
        ...
        $response->assertJsonPath("0.attributes.9.related.0.entity", ChildFakeWritableEntity::getEntityName());
         $response->assertJsonPath("0.attributes.9.related.0.type", EntityType::WRITABLE->value);
        $response->assertJsonPath("0.attributes.9.related.0.attributes.12.linked_entity",ParentFakeWritableEntity::getEntityName());

        // Validate relations one of one
        $response->assertJsonPath("0.attributes.10.name", "relations_one_of_one");
        $response->assertJsonPath("0.attributes.10.related.0.entity", AdjacentFakeWritableEntity::getEntityName());
